 ...
 * <p>
 * It encapsulates user identification and permissions. Think of it as an initialized "shell" to access the OpenCms VFS. Every call to a method here will be checked for user permissions according to
 * the <code>{@link org.opencms.file.CmsRequestContext}</code> this CmsObject instance was created with.<p>
 * <p>
 * From a JSP page running in OpenCms, use <code>{@link org.opencms.jsp.CmsJspBean#getCmsObject()}</code> to gain access to the current users CmsObject. Usually this is done with a <code>{@link
 * org.opencms.jsp.CmsJspActionElement}</code>.<p>
 * <p>
 ...
 * the name of the guest user, usually "Guest" and more formally obtained by <code>{@link org.opencms.db.CmsDefaultUsers#getUserGuest()}</code>. This will give you an initialized context with guest
 * user permissions. Then use <code>{@link CmsObject#loginUser(String, String)}</code> to log in the user you want. Obviously you need the password for the new user. You should never try to create an
 * instance of this class using the constructor, this is reserved for internal operation only.<p>
 *
 * @since 6.0.0
 */
public final class CmsObject {
     ...
     * If you want to create a new instance of a <code>{@link CmsObject}</code> in your application, use <code>{@link org.opencms.main.OpenCms#initCmsObject(String)}</code>.<p>
     *
    ...
    /**
     * Convenience method to add the site root from the current user's request context to the given resource name.<p>
     *
    ...
    /**
     * This method works just like {@link CmsObject#adjustLinks(String, String)}, but you can specify multiple source files, and the target folder is interpreted as the folder into which the source
     * files have been copied.<p>
     *
    ...
    /**
     * This method works just like {@link CmsObject#adjustLinks(String, String)}, but instead of specifying a single source and target folder, you can specify multiple sources and the corresponding
     * targets in a map of strings.
     *
     ...
     * <p>
     * If a link is found from the target folder to the source folder, then the target folder is checked if a target of the same name is found also "relative" inside the target Folder, and if so, the
     * link is changed to that "relative" target. This is mainly used to keep relative links inside a copied folder structure intact.
     * <p>
     * Example: Image we have folder /folderA/ that contains files /folderA/x1 and /folderA/y1. x1 has a link to y1 and y1 to x1. Now someone copies /folderA/ to /folderB/. So we end up with
     * /folderB/x2 and /folderB/y2. Because of the link mechanism in OpenCms, x2 will have a link to y1 and y2 to x1. By using this method, the links from x2 to y1 will be replaced by a link x2 to y2,
     * and y2 to x1 with y2 to x2.
     * <p>
     * Link replacement works for links in XML files as well as relation only type links.
     *
    ...
    /**
     * Returns a list with all sub resources of a given folder that have set the given property, matching the current property's value with the given old value and replacing it by a given new
     * value.<p>
     *
    ...
    public List<CmsResource> changeResourcesInFolderWithProperty(String resourcename, String property, String oldValue, String newValue, boolean recursive) throws CmsException {
        ...
        return m_securityManager.changeResourcesInFolderWithProperty(m_context, resource, property, oldValue, newValue, recursive);
    }

    /**
     * Changes the resource flags of a resource.<p>
     * <p>
     * The resource flags are used to indicate various "special" conditions for a resource. Most notably, the "internal only" setting which signals that a resource can not be directly requested with
     * it's URL.<p>
     *
     ...
     * <p>
     * OpenCms handles resources according to the resource type, not the file suffix. This is e.g. why a JSP in OpenCms can have the suffix ".html" instead of ".jsp" only. Changing the resource type
     * makes sense e.g. if you want to make a plain text file a JSP resource, or a binary file an image, etc.<p>
     *
     ...
     * @param type         the new resource type for this resource
     * @throws CmsException if something goes wrong
     * @deprecated Use {@link #chtype(String, I_CmsResourceType)} instead. Resource types should always be referenced either by its type class (preferred) or by type name. Use of int based resource
     ...
     * Copies a resource.<p>
     * <p>
     * The copied resource will always be locked to the current user after the copy operation.<p>
     * <p>
     ...
     * The copied resource will always be locked to the current user after the copy operation.<p>
     * <p>
     * The <code>siblingMode</code> parameter controls how to handle siblings during the copy operation.<br> Possible values for this parameter are: <br>
     * <ul>
     ...
     * <p>
     * This is used to extend the current users project with the specified resource, in case that the resource is not yet part of the project. The resource is not really copied like in a regular copy
     * operation, it is in fact only "enabled" in the current users project.<p>
     *
    ...
    /**
     * Creates a resource with the given properties and content. Will throw an exception, if a resource with the given name already exists.<p>
     *
     ...
     * @throws CmsIllegalArgumentException if the given <code>resourcename</code> is null or of length 0
     * @see #createResource(String, int, byte[], List)
     * @deprecated Use {@link #createResource(String, I_CmsResourceType)} instead. Resource types should always be referenced either by its type class (preferred) or by type name. Use of int based
     ...
     * @throws CmsException                if something goes wrong
     * @throws CmsIllegalArgumentException if the <code>resourcename</code> argument is null or of length 0
     * @deprecated Use {@link #createResource(String, I_CmsResourceType, byte[], List)} instead. Resource types should always be referenced either by its type class (preferred) or by type name. Use of
     ...
     * <p>
     * The <code>siblingMode</code> parameter controls how to handle siblings during the delete operation.<br> Possible values for this parameter are: <br>
     * <ul>
    ...
    /**
     * Checks the availability of a resource in the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.<p>
     * <p>
     ...
     * <code>{@link CmsFolder}</code>.<p>
     * <p>
     * The specified filter controls what kind of resources should be "found" during the read operation. This will depend on the application. For example, using <code>{@link
     * CmsResourceFilter#DEFAULT}</code> will only return currently "valid" resources, while using <code>{@link CmsResourceFilter#IGNORE_EXPIRATION}</code> will ignore the date release / date expired
     * information of the resource.<p>
     * <p>
     * This method also takes into account the user permissions, so if the given resource exists, but the current user has not the required permissions, then this method will return
     * <code>false</code>.<p>
     *
     ...
     * <p>
     * The detail view URI of a content element consists of its detail page URI and the detail name returned by this method.<p>
     *
     ...
     * <p>
     * The result is filtered according to the rules of the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.<p>
     *
     ...
     * Returns all file resources contained in a folder.<p>
     * <p>
     * With the <code>{@link CmsResourceFilter}</code> provided as parameter you can control if you want to include deleted, invisible or time-invalid resources in the result.<p>
     *
    ...
    /**
     * Returns the name a resource would have if it were moved to the "lost and found" folder. <p>
     * <p>
     * In general, it is the same name as the given resource has, the only exception is if a resource in the "lost and found" folder with the same name already exists. In such case, a counter is added
     * to the resource name.<p>
     *
     ...
     * @param filter   the filter to match the relation
     * @return a List containing all {@link org.opencms.relations.CmsRelation} objects for the given resource matching the given filter
     * @throws CmsException if something goes wrong
     ...
     * <p>
     * This request context is used to authenticate the user for all OpenCms operations. It also contains the request runtime settings, e.g. about the current site this request was made on.<p>
     *
     ...
     * <p>
     * If the <code>includeAttr</code> flag is set it returns also all resources associated to a given principal through some of following attributes.<p>
     *
     * <ul>
     *    <li>User Created</li>
    ...
    /**
     * Returns all child resources of a resource, that is the resources contained in a folder.<p>
     * <p>
     * With the <code>{@link CmsResourceFilter}</code> provided as parameter you can control if you want to include deleted, invisible or time-invalid resources in the result.<p>
     * <p>
     ...
     * <p>
     * If the resource root path does not start with the current site root, it is left untouched.<p>
     *
     ...
     * @param resource            the resource to check the permissions for
     * @param requiredPermissions the set of permissions to check for
     * @param checkLock           if <code>true</code> the lock status of the resource is checked for write operations and the resource needs be locked by the current user so that the test is passed,
     ...
     * <p>
     * If a resource already exists in the VFS (i.e. has the same name and same id) it is replaced by the imported resource.<p>
     * <p>
     * If a resource with the same name but a different id exists, the imported resource is (usually) moved to the "lost and found" folder.<p>
     *
     ...
     * @throws CmsException if something goes wrong
     */
    public CmsUser importUser(String id, String name, String password, String firstname, String lastname, String email, int flags, long dateCreated, Map<String, Object> additionalInfos)
            throws CmsException {

        return m_securityManager.importUser(m_context, id, name, password, firstname, lastname, email, flags, dateCreated, additionalInfos);
    }
     ...
     * <p>
     * The project "view" is determined by a set of path prefixes. If the resource starts with any one of this prefixes, it is considered to be "inside" the project.<p>
     *
     ...
     * <p>
     * This will be an exclusive, temporary lock valid only for the current users session. Usually this should not be used directly, this method is intended for the OpenCms workplace only.<p>
     *
     * @param resource the resource to lock
     * @throws CmsException if something goes wrong
     ...
     * <p>
     * A move operation in OpenCms is always a copy (as sibling) followed by a delete, this is a result of the online/offline structure of the OpenCms VFS. This way you can see the deleted
     * files/folders in the offline project, and you will be unable to undelete them.<p>
     *
     ...
     * <p>
     * This operation is used e.g. during import of resources when a resource with the same name but a different resource ID already exists in the VFS. In this case, the imported resource is moved to
     * the "lost and found" folder.<p>
     *
    ...
    /**
     * Reads all deleted (historical) resources below the given path, including the full tree below the path, if required.<p>
     * <p>
     * The result list may include resources with the same name of resources (with different id's).<p>
     * <p>
     * Use in conjunction with the {@link #restoreDeletedResource(CmsUUID)} method.<p>
     *
    ...
    /**
     * Reads a file resource (including it's binary content) from the VFS, for the given resource (this may also be an historical version of the resource).<p>
     * <p>
     * In case the input {@link CmsResource} object already is a {@link CmsFile} with contents available, it is casted to a file and returned unchanged. Otherwise the file is read from the VFS.<p>
     * <p>
     * In case you do not need the file content, use <code>{@link #readResource(String)}</code> or
     * <code>{@link #readResource(String, CmsResourceFilter)}</code> instead.<p>
     * <p>
     * No resource filter is applied when reading the resource, since we already have a full resource instance and assume we just want the content for that instance. In case you need to apply a
     * filter, use {@link #readFile(String, CmsResourceFilter)} instead.<p>
     *
    ...
    /**
     * Reads a file resource (including it's binary content) from the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.<p>
     * <p>
     * In case you do not need the file content, use <code>{@link #readResource(String)}</code> instead.<p>
     *
     ...
     * <p>
     * In case you do not need the file content, use <code>{@link #readResource(String, CmsResourceFilter)}</code> instead.<p>
     * <p>
     * The specified filter controls what kind of resources should be "found" during the read operation. This will depend on the application. For example, using <code>{@link
     * CmsResourceFilter#DEFAULT}</code> will only return currently "valid" resources, while using <code>{@link CmsResourceFilter#IGNORE_EXPIRATION}</code> will ignore the date release / date expired
     * information of the resource.<p>
    ...
    /**
     * Reads a folder resource from the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.<p>
     *
     ...
     * @return the list of all resource names (root paths), as <code>{@link String}</code> objects that define the "view" of the given project
     * @throws CmsException if something goes wrong
     */
     ...
     * <p>
     * This method is more efficient then using <code>{@link CmsObject#readPropertyObject(String, String, boolean)}</code> if you already have an instance of the resource to look up the property
     * from.<p>
     *
     * @param resource the resource where the property is attached to
     * @param property the property name
     * @param search   if true, the property is searched on all parent folders of the resource, if it's not found attached directly to the resource
     ...
     * <p>
     * This method is more efficient then using <code>{@link CmsObject#readPropertyObjects(String, boolean)}</code> if you already have an instance of the resource to look up the property from.<p>
     * <p>
     * If the <code>search</code> parameter is <code>true</code>, the properties of all parent folders of the resource are also read. The results are merged with the properties directly attached to
     * the resource. While merging, a property on a parent folder that has already been found will be ignored. So e.g. if a resource has a property "Title" attached, and it's parent folder has the
     * same property attached but with a different value, the result list will contain only the property with the value from the resource, not form the parent folder(s).<p>
     *
     * @param resource the resource where the property is mapped to
     * @param search   if <code>true</code>, the properties of all parent folders of the resource are merged with the resource properties.
    ...
    /**
     * Reads a resource from the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.<p>
     * <p>
     * A resource may be of type <code>{@link CmsFile}</code> or
     * <code>{@link CmsFolder}</code>. In case of
     * a file, the resource will not contain the binary file content. Since reading the binary content is a cost-expensive database operation, it's recommended to work with resources if possible, and
     * only read the file content when absolutely required. To "upgrade" a resource to a file, use <code>{@link #readFile(CmsResource)}</code>.<p>
     *
    ...
    /**
     * Reads the historical resource with the given version for the resource given the given structure id.<p>
     * <p>
     * A resource may be of type <code>{@link CmsFile}</code> or
     * <code>{@link CmsFolder}</code>. In case of a file, the resource will not
     * contain the binary file content. Since reading the binary content is a cost-expensive database operation, it's recommended to work with resources if possible, and only read the file content
     * when absolutely required. To "upgrade" a resource to a file, use
     * <code>{@link #readFile(CmsResource)}</code>.<p>
     * <p>
     * Please note that historical versions are just generated during publishing, so the first version with version number 1 is generated during publishing of a new resource (exception is a new
     * sibling, that may also contain some relevant versions of already published siblings) and the last version available is the version of the current online resource.<p>
     *
     * @param structureID the structure ID of the resource to read
     ...
     * @return a list of all <code>{@link CmsResource}</code> objects that have a value set for the specified property.
     * @throws CmsException if something goes wrong
     */
     ...
     * @param propertyDefinition the name of the property to check for
     * @return all <code>{@link CmsResource}</code> objects that have a value set for the specified property in the given path.
     * @throws CmsException if something goes wrong
     */
    ...
    /**
     * Reads all resources that have a value (containing the specified value) set for the specified property in the given path.<p>
     * <p>
     * Both individual and shared properties of a resource are checked.<p>
     * <p>
     * If the <code>value</code> parameter is <code>null</code>, all resources having the given property set are returned.<p>
     * <p>
     * Will use the {@link CmsResourceFilter#ALL} resource filter.<p>
     *
     * @param path               the folder to get the resources with the property from
    ...
    public List<CmsResource> readResourcesWithProperty(String path, String propertyDefinition, String value) throws CmsException {
        ...
        return m_securityManager.readResourcesWithProperty(m_context, resource, propertyDefinition, value, CmsResourceFilter.ALL);
    }
    ...
    /**
     * Returns a list of all siblings of the specified resource, the specified resource being always part of the result set.<p>
     *
     * @param resource the resource
     * @param filter   a resource filter
     * @return a list of <code>{@link CmsResource}</code>s that are siblings to the specified resource, including the specified resource itself.
     * @throws CmsException if something goes wrong
     */
    ...
    public List<CmsResource> readSiblingsForResourceId(CmsUUID resourceId, CmsResourceFilter filter) throws CmsException {
        ...
        CmsResource pseudoResource = new CmsResource(null, resourceId, null, 0, false, 0, null, null, 0, null, 0, null, 0, 0, 0, 0, 0, 0);
     ...
     * <p>
     * This is used to reduce the current users project with the specified resource, in case that the resource is already part of the project. The resource is not really removed like in a regular copy
     * operation, it is in fact only "disabled" in the current users project.<p>
     *
    ...
    /**
     * Renames a resource to the given destination name, this is identical to a <code>move</code> operation.<p>
     *
     ...
     * @param properties   the new properties of the resource
     * @throws CmsException if something goes wrong
     * @deprecated Use {@link #replaceResource(String, I_CmsResourceType, byte[], List)} instead. Resource types should always be referenced either by its type class (preferred) or by type name. Use
     * of int based resource type references will be discontinued in a future OpenCms release.
     */
    @Deprecated
     ...
     * @param parentGroupName the name of the parent group to set, or <code>null</code> if the parent group should be deleted.
     * @throws CmsException if operation was not successful
     */
     ...
     * <p>
     * Only resources that have already been published once can be undeleted, if a "new" resource is deleted it can not be undeleted.<p>
     *
    ...
    /**
     * Undoes all changes to a resource by restoring the version from the online project to the current offline project.<p>
     *
     ...
     * <p>
     * Applies only to resources of type <code>{@link CmsFile}</code> i.e. resources that have a binary content attached.<p>
     * <p>
     * Certain resource types might apply content validation or transformation rules before the resource is actually written to the VFS. The returned result might therefore be a modified version from
     * the provided original.<p>
     *
     ...
     * <p>
     * The project with the given id will be completely overridden by the given data.<p>
     *
     ...
     * @throws CmsException if resource type is set to folder, or if the user has not the rights to write the file header.
     */
     ...
     * <p>
     * The first name from the given name sequence which is not already mapped to another resource will be used for the URL name mapping.<p>
     *
     ...
     * Writes a new URL name mapping for a given resource.<p>
     * <p>
     * This method uses {@link CmsNumberSuffixNameSequence} to generate a sequence of name candidates from the given base name.<p>
     *
     ...
     * <p>
     * The user with the given id will be completely overriden by the given data.<p>
     *
    ...
    /**
     * Convenience method to return the initialized resource type instance for the given id.<p>
     *
     ...
     * Locks a resource.<p>
     * <p>
     * The <code>type</code> parameter controls what kind of lock is used.<br> Possible values for this parameter are: <br>
     * <ul>
